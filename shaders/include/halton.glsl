...
{
    return ((halton2Inverse(x % 256, 8) * 76545 + halton3Inverse(y % 256, 6) * 110080) % (76545+110080)) + i * 186624;
}

void haltonInit(inout HaltonState hState, int x, int y, int path, int numPaths, int frameId, int loop)
{
    hState.dimension = frameId % 32;
    hState.sequenceIndex = haltonIndex(x, y, frameId);//(frameId * numPaths + path) % (loop * numPaths));
}
...
float haltonNext(inout HaltonState hState)
{
    float s = haltonSample(hState.dimension, hState.sequenceIndex);
    hState.dimension = (hState.dimension + 1) % 32;
    return s;
}
