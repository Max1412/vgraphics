...
namespace vg
{
    ...
    class PBRDeferredApp : public BaseApp
    {
    public:
        PBRDeferredApp() :
			...
			m_PBRscene("pica_pica_-_mini_diorama_01/scene.gltf")
		{
            //createRenderPass();
        ...
        {
            const auto vertShaderCode = Utility::readFile("deferredPBR/gbuffer.vert.spv");
            const auto fragShaderCode = Utility::readFile("deferredPBR/gbufferPBR.frag.spv");
        ...
        void createFullscreenLightingPipeline()
        {
            ...
            const auto vertShaderCode = Utility::readFile("deferredPBR/fullscreen.vert.spv");
            const auto fragShaderCode = Utility::readFile("deferredPBR/fullscreenLightingPBR.frag.spv");
        ...
        void createSceneInformation(const char * foldername)
        {
            ...
            // load all images
            std::vector<ImageLoadInfo> loadedImages(m_PBRscene.getIndexedBaseColorTexturePaths().size() + m_PBRscene.getIndexedMetallicRoughnessTexturePaths().size());
            stbi_set_flip_vertically_on_load(true);

			// base color (rgba?) //TODO alpha?
#pragma omp parallel for
            for (int i = 0; i < static_cast<int>(m_PBRscene.getIndexedBaseColorTexturePaths().size()); i++)
            {
                auto path = g_resourcesPath;
                const auto name = std::string(std::string(foldername) + m_PBRscene.getIndexedBaseColorTexturePaths().at(i).second);
                path.append(name);
			...
			// metallic roughness - TODO which is which? channels ok?
#pragma omp parallel for
            for (int i = static_cast<int>(m_PBRscene.getIndexedBaseColorTexturePaths().size()); i < static_cast<int>(loadedImages.size()); i++)
            {
                auto path = g_resourcesPath;
                const auto name = std::string(std::string(foldername) + m_PBRscene.getIndexedMetallicRoughnessTexturePaths().at(i - m_PBRscene.getIndexedBaseColorTexturePaths().size()).second);
                path.append(name);
                loadedImages.at(i).pixels = stbi_load(path.string().c_str(), &loadedImages.at(i).texWidth, &loadedImages.at(i).texHeight, &loadedImages.at(i).texChannels, STBI_rgb_alpha);
                loadedImages.at(i).mipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(loadedImages.at(i).texWidth, loadedImages.at(i).texHeight)))) + 1;
            }
        ...
        {
            m_vertexBufferInfo = fillBufferTroughStagedTransfer(m_PBRscene.getVertices(), vk::BufferUsageFlagBits::eVertexBuffer);
        }

        void createIndexBuffer()
        {
            m_indexBufferInfo = fillBufferTroughStagedTransfer(m_PBRscene.getIndices(), vk::BufferUsageFlagBits::eIndexBuffer);
        }

        void createIndirectDrawBuffer()
        {
            m_indirectDrawBufferInfo = fillBufferTroughStagedTransfer(m_PBRscene.getDrawCommandData(), vk::BufferUsageFlagBits::eIndirectBuffer | vk::BufferUsageFlagBits::eStorageBuffer);
        }

        void createPerGeometryBuffers()
        {
            m_modelMatrixBufferInfo = fillBufferTroughStagedTransfer(m_PBRscene.getModelMatrices(), vk::BufferUsageFlagBits::eStorageBuffer);
        }

        void createMaterialBuffer()
        {
            m_materialBufferInfo = fillBufferTroughStagedTransfer(m_PBRscene.getMaterials(), vk::BufferUsageFlagBits::eStorageBuffer);
        }
        ...
        void createAllCommandBuffers()
        {
            ...
            for (size_t i = 0; i < m_gbufferSecondaryCommandBuffers.size(); i++)
            {
                ...
                m_gbufferSecondaryCommandBuffers.at(i).drawIndexedIndirect(m_indirectDrawBufferInfo.m_Buffer, 0, static_cast<uint32_t>(m_PBRscene.getDrawCommandData().size()),
                    sizeof(std::decay_t<decltype(*m_PBRscene.getDrawCommandData().data())>));
